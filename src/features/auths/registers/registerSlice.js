...
export const registerAsync = createAsyncThunk("register", async (datas) => {
  const urlData = urlJson.method.find(item => item.typeUrl === 'register')
  ...
  const data = {
    ...
    url:
      urlData.url ||
      ` http:///localhost:3001/api/auth/auth/register`,
    datas,
  };
  console.log("🚀 ~ file: registerSlice.js ~ line 40 ~ registerAsync ~ data", data)
  const dataIndexAxios = await IndexAxios(data);
  console.log("🚀 ~ file: registerSlice.js ~ line 42 ~ registerAsync ~ dataIndexAxios", dataIndexAxios)
   if (dataIndexAxios) {
...
export const selectRegister = (state) => state.login;
